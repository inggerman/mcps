...
echo "=== 1. CREATE KANIKO BUILD JOB ==="
    ...
    spec:
      ...
      - name: clone
        ...
        - |
          ...
          echo "=== Dockerfile.api line 1 ==="
          head -1 /workspace/source/Dockerfile.api
          echo "=== Dockerfile.worker line 1 ==="
          head -1 /workspace/source/Dockerfile.worker
        volumeMounts:
        - name: workspace
          mountPath: /workspace
        resources:
...
echo "=== 2. POLL EVERY 30s FOR 10min ==="
...
echo "=== 4. VERIFY IMAGES IN HARBOR ==="
POD=$(kubectl get pods -n gitea-runner -l job-name=copy-python-base -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
if [ -n "$POD" ]; then
  kubectl exec -n gitea-runner $POD -- crane ls harbor-registry.harbor.svc.cluster.local:5000/ghl/agents-platform-api --insecure 2>&1 || true
  kubectl exec -n gitea-runner $POD -- crane ls harbor-registry.harbor.svc.cluster.local:5000/ghl/agents-platform-worker --insecure 2>&1 || true
fi
echo ""
