...
echo "=== 3. CREATE KANIKO JOB (IP-based clone) ==="
    ...
    spec:
      ...
      - name: clone
        ...
        - |
          ...
          # Fix: add pip install and PYTHONPATH after COPY . .
          sed -i '/^COPY \. \.$/a RUN pip install --no-cache-dir .' Dockerfile.api
          sed -i '/^COPY \. \.$/a ENV PYTHONPATH=/app' Dockerfile.api
          sed -i '/^COPY \. \.$/a RUN pip install --no-cache-dir .' Dockerfile.worker
          sed -i '/^COPY \. \.$/a ENV PYTHONPATH=/app' Dockerfile.worker
...
echo "=== 4. POLL EVERY 30s FOR 10min ==="
for i in $(seq 1 20); do
  ...
  if [ "$PHASE" = "Running" ]; then
    ...
    echo "=== FINAL ==="
    echo "--- init ---"
    kubectl logs -n gitea-runner $POD -c clone 2>&1 | tail -20
    echo "--- api ---"
