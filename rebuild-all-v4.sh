...
echo "=== 2. CREATE KANIKO JOB (syntax fix + MCP images) ==="
    ...
    spec:
      ...
      - name: clone
        ...
        - |
          ...
          # Fix syntax error in slack_notifier.py
          sed -i 's/{context\[:2900}/{context[:2900]/g' notifications/slack_notifier.py
          echo "=== Fixed slack_notifier.py ==="
          grep -n 'context\[:2900' notifications/slack_notifier.py
          ...
          # Create Dockerfile for mcp-web-search
          ...
          # Create Dockerfile for mcp-source-validator
...
for i in $(seq 1 24); do
  ...
  if [ "$PHASE" = "Running" ]; then
    ...
    echo "  web-search:"
    kubectl logs -n gitea-runner $POD -c kaniko-web-search --tail=2 2>&1
    echo "  source-validator:"
    kubectl logs -n gitea-runner $POD -c kaniko-source-validator --tail=2 2>&1
    ...
    echo "--- web-search ---"
    kubectl logs -n gitea-runner $POD -c kaniko-web-search --tail=10 2>&1
    echo "--- source-validator ---"
    kubectl logs -n gitea-runner $POD -c kaniko-source-validator --tail=10 2>&1
    break
  fi
  echo ""
done
