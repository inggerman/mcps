...
# Write the Job manifest to a temp file first to avoid heredoc-in-heredoc issues
    ...
    spec:
      ...
      - name: clone
        ...
        - |
          ...
          printf '"""API Webhook — endpoints para respuestas externas al pipeline."""\nfrom fastapi import FastAPI\nfrom agents_platform.api.webhook import router\napp = FastAPI(title="Autonomous Agents Platform API", version="0.1.0")\napp.include_router(router)\n__all__ = ["router", "app"]\n' > api/__init__.py
...
echo "=== 2. APPLY JOB ==="
kubectl apply -f /tmp/kaniko-job.yaml 2>&1
echo ""
