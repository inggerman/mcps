...
echo "=== 2. UPDATE DOCKER CONFIG FOR INTERNAL HARBOR ==="
...
echo "=== 3. CREATE KANIKO BUILD JOB ==="
  ...
  template:
    ...
    spec:
      ...
      - name: kaniko-api
        ...
        - "--destination=${HARBOR_INTERNAL}/ghl/agents-platform-api:latest"
        - "--insecure"
        - "--skip-tls-verify"
        volumeMounts:
        - name: workspace
      ...
      - name: kaniko-worker
        ...
        - "--context=dir:///workspace/source"
        - "--destination=${HARBOR_INTERNAL}/ghl/agents-platform-worker:latest"
        - "--insecure"
        - "--skip-tls-verify"
        volumeMounts:
        - name: workspace
...
echo "Pod: $POD"
kubectl logs -n argocd $POD -c clone --tail=10 2>&1
echo ""
